    ...
    if (empty($categoria_id)) {
        ...
        try {
            ...
            if (!$stmt->fetch()) {
                ...
                // Atualiza os dados da transação para exibir
                $stmt = $pdo->prepare("
                    SELECT t.*, c.nome as categoria_nome, c.tipo as categoria_tipo
                    FROM transacoes t
                    LEFT JOIN categorias c ON t.categoria_id = c.id
...
<head>
    ...
    <title>Editar Transação - FinControl</title>
    ...
    <section class="page-container">
        <div class="container">
            <div class="page-header">
                <h1>
                    <i class="fas fa-edit" style="color: #f59e0b;"></i> Editar Transação
                    <small>Atualize os dados da transação</small>
                </h1>
                <a href="transactions.php" class="btn btn-outline">
            ...
            <div class="form-container">
                <div class="transaction-id">
                    <i class="fas fa-hashtag"></i> ID: #<?php echo $transacao['id']; ?>
                </div>
                ...
                <form method="POST" id="formTransacao">
                    ...
                    <div class="form-group">
                        ...
                        <div class="tipo-selector">
                            <button type="button" class="tipo-btn <?php echo $transacao['tipo'] === 'despesa' ? 'active-despesa' : ''; ?>" id="btnDespesa" onclick="setTipo('despesa')">
                                <i class="fas fa-arrow-down"></i> Despesa
                            </button>
                            <button type="button" class="tipo-btn <?php echo $transacao['tipo'] === 'receita' ? 'active-receita' : ''; ?>" id="btnReceita" onclick="setTipo('receita')">
                                <i class="fas fa-arrow-up"></i> Receita
                            </button>
                        </div>
                        <input type="hidden" name="tipo" id="tipoInput" value="<?php echo $transacao['tipo']; ?>">
                    </div>
                        ...
                        <select name="categoria_id" id="categoria_id" required>
                            ...
                            <optgroup label="📈 Receitas">
                                <?php foreach ($categorias_receita as $cat): ?>
                                    <option value="<?php echo $cat['id']; ?>" data-tipo="receita" <?php echo $cat['id'] == $transacao['categoria_id'] ? 'selected' : ''; ?>>
                            ...
                            <optgroup label="📉 Despesas">
                                <?php foreach ($categorias_despesa as $cat): ?>
                                    <option value="<?php echo $cat['id']; ?>" data-tipo="despesa" <?php echo $cat['id'] == $transacao['categoria_id'] ? 'selected' : ''; ?>>
                                        <?php echo htmlspecialchars($cat['nome']); ?>
                                    </option>
                                <?php endforeach; ?>
                            </optgroup>
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-icon">
                                ...
                                <input type="text" id="valor" name="valor" 
                                       value="<?php echo formatCurrency($transacao['valor']); ?>"
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <select name="forma_pagamento" id="forma_pagamento">
                                <option value="dinheiro" <?php echo $transacao['forma_pagamento'] === 'dinheiro' ? 'selected' : ''; ?>>💰 Dinheiro</option>
                                <option value="cartao_credito" <?php echo $transacao['forma_pagamento'] === 'cartao_credito' ? 'selected' : ''; ?>>💳 Cartão de Crédito</option>
                                <option value="cartao_debito" <?php echo $transacao['forma_pagamento'] === 'cartao_debito' ? 'selected' : ''; ?>>💳 Cartão de Débito</option>
                                <option value="pix" <?php echo $transacao['forma_pagamento'] === 'pix' ? 'selected' : ''; ?>>📱 PIX</option>
                                <option value="boleto" <?php echo $transacao['forma_pagamento'] === 'boleto' ? 'selected' : ''; ?>>📄 Boleto</option>
                                <option value="transferencia" <?php echo $transacao['forma_pagamento'] === 'transferencia' ? 'selected' : ''; ?>>🏦 Transferência</option>
                            </select>
                        </div>
                        <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="observacao" name="observacao" 
                                  placeholder="Observações adicionais (opcional)"><?php echo htmlspecialchars($transacao['observacao'] ?? ''); ?></textarea>
                    </div>
